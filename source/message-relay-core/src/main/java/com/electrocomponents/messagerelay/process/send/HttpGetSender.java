 ...
 * *******************************************************************************************************************
 * * GEN-1302 * e0161085 *  12-Oct-2010 * Tidied Code.
 * *******************************************************************************************************************
 *
 * @author e0180383
 */
public class HttpGetSender {
     ...
     * Log4J.
     */
    private static Log log = LogFactory.getLog(HttpGetSender.class);
     ...
     */
     public BusinessMessage processMessage(final BusinessMessage outboundMessage) throws ElectroException {
        ...
        if (log.isDebugEnabled()) {
          ...
          log.debug("processMessage : Number of FORM GET parameters to be sent = " + outboundMessage.getPostParameters().size());
        ...
        try {
           ...
           while (paramIterator.hasNext()) {
              ...
              if (paramName != null) {
                  get.getParams().setParameter(paramName, paramValue);
                 //get.addParameter(paramName, paramValue);
                 if (log.isDebugEnabled()) {
           ...
           if (httpResponse.getStatusLine().getStatusCode() != 200) {
               ...
               String error = "processMessage : An error occurred executing the FORM GET "
                     + "to " + target + ", the HTTP R/C was " + responseCode + " the error message was " + responseMess;
               log.fatal(error);
               throw new ElectroException("HttpPostSender." + error);
           } else {
